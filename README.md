...
Once this component is showing a lost of blog posts we are able to navigate to an individual blog post by using the link tag provided by React router. The Link-tag is used just like any other component, and it produces just a normal anchor tag on the page.
...
####Show####
The post show page is another component that fetched it's own data and the one twist here is that it needs a parameter out of the URL to figure out what individual post it wants to fetch. This componet is also in charge of deleting posts and after a user succesfully deletes a post it navigates back to the index route.

####New####

One of the most interesting aspects of this component is redux-form. At the bottom if the file there is configuration for this form: we named the form and we told it witch fields it was going to be in charge of. We also defined a validation function.
...
We also use redux-form for handling validation. Specifically we made sure that we show an error for each individual input, only after the user has interacted with the input in some fashion.
Finally we made sure we highlighted the entire field and the error message in red, if the user has touched it and if the particular field is not valid.
